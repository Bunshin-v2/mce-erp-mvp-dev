...
export const TenderIntakeWizard: React.FC<TenderIntakeWizardProps> = ({ tenderId, tenderTitle, clientName, initialTemplate, onComplete, onCancel }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            const { error } = await supabase.rpc('generate_tender_checklist', {
    ...
    return (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
            <div className={`border border-white/5 w-full max-w-4xl rounded-3xl shadow-5xl flex flex-col overflow-hidden animate-in zoom-in-95 duration-300 transition-all ${step === 3 ? 'bg-black scale-105 border-emerald-500/20' : 'bg-[#0f0f11]'}`}>
                ...
                <div className="bg-zinc-900/10 p-8 border-b border-white/5 flex justify-between items-start">
                    <div>
                        {tenderTitle ? (
                            <>
                                <h2 className="text-xl font-black text-white uppercase tracking-tighter mb-1">{tenderTitle}</h2>
                            ...
                            <p className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-500">Registry Calibration Framework v2.0</p>
                        )}
                    </div>
                    {step !== 3 && (
                ...
                <div className="p-12 min-h-[500px] bg-black/40 flex flex-col relative">
                    ...
                    {step === 1 && (
                        <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
                            <div>
                                <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-400 mb-8 flex items-center gap-3">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                    {TEMPLATES.map(t => (
                                        ...
                                        >
                                            ...
                                            <h5 className={`text-base font-black uppercase tracking-widest leading-tight mb-4 ${selectedTemplate === t.id ? 'text-white' : 'text-zinc-400 group-hover:text-zinc-200'}`}>{t.name}</h5>
                                            <p className="text-[10px] font-mono text-zinc-600 uppercase tracking-widest font-bold group-hover:text-zinc-500 transition-colors">
                    ...
                    {step === 2 && template && (
                        <div className="space-y-8 animate-in fade-in slide-in-from-right-8 duration-500 h-full flex flex-col">
                            <h4 className="text-[11px] font-black uppercase tracking-[0.3em] text-zinc-400 flex items-center gap-3">
                                <span className="w-6 h-px bg-zinc-700"></span>
                            ...
                            <div className="bg-black border border-white/5 rounded-xl p-8 overflow-y-auto custom-scrollbar flex flex-col divide-y divide-zinc-900 shadow-inner flex-1">
                                {template.sections.map((section, idx) => (
                                    <div key={idx} className="py-6 first:pt-0 last:pb-0">
                                        <h5 className="text-emerald-500 font-black text-[10px] mb-5 uppercase tracking-[0.2em] italic flex items-center">
                                            <div className="w-1.5 h-1.5 bg-emerald-500 rounded-full mr-3 shadow-[0_0_8px_#10b981]"></div>
                                            {section.title}
                                        </h5>
                                        <div className="space-y-3 ml-6">
                                            {section.items.map((item, i) => (
                                                <div key={i} className="flex items-center text-[10px] font-mono font-bold text-zinc-500 uppercase tracking-tight group hover:text-zinc-200 transition-colors cursor-default">
                    ...
                    {step === 3 && (
                        <div className="flex flex-col items-center justify-center h-full animate-in zoom-in-95 duration-700 space-y-10 py-10">
                            ...
                            <div className="text-center space-y-4">
                                ...
                                <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-[0.2em] max-w-sm mx-auto leading-relaxed">
                            ...
                            <div className="pt-8">
                                <button
                                    onClick={onComplete}
                                    className="bg-white text-black hover:bg-zinc-200 px-12 py-4 rounded-xl text-xs font-black uppercase tracking-[0.25em] shadow-[0_0_30px_rgba(255,255,255,0.2)] transition-all hover:scale-105"
                                >
                            ...
                            <div className="absolute bottom-0 w-full flex justify-between items-center pt-12 opacity-50">
                                <span className="text-[9px] font-mono text-zinc-600 uppercase tracking-widest font-bold flex items-center gap-2">
                                    <span className="w-2 h-2 rounded-full bg-zinc-800"></span> Auth: L3-Executive
                                </span>
                                <button className="text-[9px] font-bold text-rose-900/50 hover:text-rose-500 uppercase tracking-widest transition-colors">
                                    Hard Reset Registry
                                </button>
                            </div>
                        </div>
                ...
                {step < 3 && (
                    <div className="p-8 border-t border-white/5 flex justify-between items-center bg-zinc-900/30 backdrop-blur-md">
                        <span className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.4em]">Step 0{step} / 02</span>
                        <div className="flex items-center gap-4">
                            {step > 1 && (
                                <button
                                    onClick={() => setStep(step - 1)}
                                    className="text-zinc-500 hover:text-white px-6 py-2 text-[10px] font-black uppercase tracking-widest transition-all hover:-translate-x-1"
                                >
                                    Back
                                </button>
                            )}
                            <button
                                ...
                                className={`
                            px-10 py-3 rounded-lg text-[10px] font-black uppercase tracking-[0.2em] flex items-center shadow-2xl transition-all disabled:opacity-30 disabled:cursor-not-allowed
                            ${loading
                                        ? 'bg-zinc-800 text-zinc-500 cursor-wait'
                                        : 'bg-[#00dc82] text-black hover:bg-emerald-400 hover:scale-105 hover:shadow-[0_0_20px_rgba(0,220,130,0.4)]'
                                    }
                        `}
                            >
