    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex justify-between items-center">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest pl-8">Organize your matrix nodes with custom labels</p>
        </div>
        <button
          onClick={onAddCategory}
          className="bg-blue-600 text-white px-8 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-500 transition-all shadow-[0_0_20px_rgba(37,99,235,0.3)]"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         <GlassPanel className="p-8 relative overflow-hidden group">
            ...
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-2 block">Total Categories</span>
            <h3 className="text-5xl font-black text-white font-mono tracking-tighter">{categories.length}</h3>
         </GlassPanel>
         <GlassPanel className="p-8 relative overflow-hidden group">
            ...
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-2 block">Categorized Tasks</span>
            <h3 className="text-5xl font-black text-white font-mono tracking-tighter">{categorizedTasksCount}</h3>
         </GlassPanel>
         <GlassPanel className="p-8 relative overflow-hidden group">
            <Star className="absolute -right-4 -bottom-4 w-24 h-24 text-amber-500/5 group-hover:text-amber-500/10 transition-colors" />
            <span className="text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mb-2 block">Most Used</span>
            <h3 className="text-xl font-bold text-white uppercase tracking-tight mt-2">{mostUsedCat?.name || 'N/A'}</h3>
            <p className="text-[10px] font-mono font-bold text-zinc-600 uppercase mt-1">{mostUsedCat ? getTasksCount(mostUsedCat.id) : 0} nodes linked</p>
         </GlassPanel>
      </div>
      ...
      <div className="space-y-4 pt-4">
         <div className="flex items-center gap-3 px-2">
            ...
            <h4 className="text-[10px] font-black text-zinc-500 uppercase tracking-widest">All Categories Registry</h4>
                     ...
                     <tr key={cat.id} className="hover:bg-white/[0.02] transition-colors group">
                        <td className="p-6">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                 <div className="font-bold text-zinc-200 text-sm group-hover:text-white transition-colors uppercase tracking-tight">{cat.name}</div>
                                 <span className="text-[8px] font-mono text-zinc-700 uppercase tracking-widest font-bold">HEX_{cat.color.toUpperCase()}</span>
                              </div>
                           </div>
                        </td>
                        <td className="p-6 text-center">
                           <span className="text-lg font-black text-white font-mono tracking-tighter">{getTasksCount(cat.id)}</span>
                        </td>
                        <td className="p-6 font-mono text-[10px] font-bold text-zinc-600 uppercase">
                     ...
                     <tr>
                        <td colSpan={4} className="p-32 text-center">
                           <div className="flex flex-col items-center justify-center space-y-6 opacity-20 filter grayscale">
                              ...
                              <p className="text-[11px] font-black uppercase tracking-[0.4em] text-zinc-500">Registry Is Empty</p>
                           </div>
                        </td>
                     </tr>
                  )}
