         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {stats.map((stat) => (
               <div key={stat.label} className="bg-[var(--surface-layer)] border border-[var(--surface-border)] rounded p-5 group transition-colors hover:border-white/10">
                  <p className="text-xs font-bold italic text-zinc-600 font-sans">{stat.label}</p>
         ...
         <div className="grid grid-cols-1 gap-6">
            <div className="bg-[var(--surface-layer)] border border-[var(--surface-border)] rounded-lg overflow-hidden transition-all">
               <div className="px-5 py-4 bg-[var(--surface-base)] border-b border-[var(--surface-border)] flex justify-between items-center">
                  <h4 className="font-extrabold text-zinc-500 text-xs flex items-center font-sans">
                     <Clock size={14} className="mr-2 text-zinc-600" /> Executive Workflow
                  </h4>
                  <div className="flex space-x-2">
                     <button className="text-xs text-[var(--color-critical)] font-bold italic hover:text-white transition-colors px-2 py-0.5 border border-zinc-800 rounded font-sans">Add Signal</button>
                     <button className="text-xs text-zinc-600 font-bold italic hover:text-zinc-300 transition-colors font-sans">Archive Done</button>
                  </div>
               </div>
               ...
               <div className="divide-y divide-[var(--surface-border)]">
                  {alerts.length === 0 ? (
                     <div className="p-8 text-center text-zinc-600 font-bold italic text-xs bg-[var(--surface-base)]">
                        [ZERO-STATE] / NO DISRUPTIONS DETECTED
                     </div>
                  ) : (
                     alerts.slice(0, 3).map((alert, i) => (
                        <div key={i} className="p-5 hover:bg-glass transition-colors flex items-start justify-between group cursor-pointer border-l-2 border-l-transparent hover:border-l-white">
                           <div className="flex items-start space-x-5">
                              <button className="mt-1 text-zinc-600 hover:text-emerald-500 transition-colors">
                                 <CheckCircle2 size={18} />
                              </button>
                              <div>
                                 <p className="text-[14px] font-bold italic text-white tracking-tight group-hover:text-zinc-200 transition-colors font-sans">{alert.title}</p>
                                 <div className="flex items-center space-x-4 mt-2">
                                    <span className={`text-xs font-bold italic font-sans ${alert.priority === 'critical' ? 'text-[var(--color-critical)] motion-critical' : 'text-zinc-500'
                                       }`}>{alert.priority} • SIGNAL</span>
                                    <span className="text-xs text-zinc-600 flex items-center font-mono">
