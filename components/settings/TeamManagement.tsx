...
export const TeamManagement: React.FC = () => {
  ...
  const getTierBadge = (tier: string) => {
    switch(tier) {
      case 'L4': return <span className="flex items-center space-x-1 px-2 py-0.5 rounded bg-rose-500/20 text-rose-400 border border-rose-500/30 text-[10px] font-bold italic tracking-wider"><ShieldAlert size={10} /> <span>L4 Platinum</span></span>;
      case 'L3': return <span className="flex items-center space-x-1 px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 text-[10px] font-bold italic tracking-wider"><Crown size={10} /> <span>L3 Gold</span></span>;
      case 'L2': return <span className="flex items-center space-x-1 px-2 py-0.5 rounded bg-blue-500/20 text-blue-400 border border-blue-500/30 text-[10px] font-bold italic tracking-wider"><Shield size={10} /> <span>L2 Silver</span></span>;
      default: return <span className="flex items-center space-x-1 px-2 py-0.5 rounded bg-slate-500/20 text-slate-200 border border-slate-500/30 text-[10px] font-bold italic tracking-wider"><User size={10} /> <span>L1 Bronze</span></span>;
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-[var(--surface-layer)] backdrop-blur-xl p-5 rounded-2xl border border-[var(--surface-border)] shadow-sm">
         <div>
            <h3 className="text-lg font-bold italic text-white flex items-center">
         ...
         <div className="relative group">
            <Search className="absolute left-3 top-2.5 text-zinc-500 group-focus-within:text-[var(--color-info)] transition-colors" size={16} />
            <input 
              type="text" 
              placeholder="Search team members..." 
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="bg-[var(--surface-base)] border border-[var(--surface-border)] rounded-xl pl-10 pr-4 py-2 text-sm text-white focus:outline-none focus:ring-2 focus:ring-[var(--color-info)] transition-all w-full md:w-64"
            />
         </div>
      </div>
      ...
      <div className="bg-[var(--surface-layer)] backdrop-blur-xl border border-[var(--surface-border)] rounded-2xl shadow-sm overflow-hidden">
         <div className="grid grid-cols-12 gap-4 px-6 py-4 bg-[var(--surface-base)] border-b border-[var(--surface-border)] text-[10px] font-bold italic text-zinc-500 tracking-wider">
            ...
            {filteredUsers.map((user) => (
               <div key={user.id} className="grid grid-cols-12 gap-4 px-6 py-4 items-center hover:bg-[var(--surface-elevated)] transition-colors group">
                  ...
                  <div className="col-span-4 flex items-center space-x-3">
                     <div className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center text-white font-bold italic border border-zinc-700 shadow-inner">
                        {user.name.charAt(0)}
                     </div>
                     <div>
                        <p className="text-sm font-bold italic text-white group-hover:text-[var(--color-info)] transition-colors">{user.name}</p>
                  ...
                  <div className="col-span-3">
                     <div className="space-y-2">
                        ...
                        <select 
                           ...
                           className="w-full bg-zinc-900 border border-zinc-700 text-white text-xs rounded px-2 py-1 focus:outline-none focus:border-[var(--color-info)] cursor-pointer"
                        >
                           {ROLES.map(role => (
