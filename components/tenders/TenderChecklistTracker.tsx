...
export const TenderChecklistTracker: React.FC<TenderChecklistTrackerProps> = ({ tenderId }) => {
  ...
  const handleAutoSuggest = async (_req: Requirement) => {
    alert('Auto-suggest is not enabled yet.');
  };
    ...
    <div className="space-y-8 font-sans w-full max-w-7xl mx-auto">
      ...
      {linkingReqId && (
        <div className="fixed inset-0 bg-black/90 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
          <div className="bg-[var(--surface-base)] border border-white/10 w-full max-w-lg rounded-2xl shadow-5xl overflow-hidden">
            <div className="p-6 border-b border-white/5 flex justify-between items-center bg-zinc-900/50">
              <h3 className="text-sm font-bold italic text-white tracking-widest">Vault_Artifact_Sync</h3>
              <button onClick={() => setLinkingReqId(null)}><X size={20} className="text-zinc-500 hover:text-white" /></button>
            </div>
            <div className="max-h-[400px] overflow-y-auto p-2 custom-scrollbar">
              {docsLoading ? (
                <div className="p-12 text-center text-zinc-600 font-mono text-[10px] tracking-[0.3em]">Scanning_Vault...</div>
              ) : (
                <div className="space-y-1">
                  {availableDocs.map(doc => (
                    ...
                    >
                      <FileText size={18} className="text-zinc-600 group-hover:text-[var(--color-success)]" />
                      <div>
                        <p className="text-xs font-bold italic text-zinc-300 group-hover:text-white">{doc.title}</p>
                        <p className="text-[9px] text-zinc-600 tracking-widest mt-1 font-bold italic">{doc.category}</p>
                      </div>
                    </button>
                  ))}
                </div>
