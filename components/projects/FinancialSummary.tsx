            ...
            {metrics.map((metric, idx) => (
                <GlassPanel key={idx} variant="base" className="p-4 relative overflow-hidden group hover:bg-white/[0.03] transition-colors">
                    ...
                    <div className="flex items-center justify-between mb-3">
                        <span className="text-[9px] font-bold italic text-zinc-500 tracking-widest">{metric.label}</span>
                    ...
                    <div className="flex flex-col">
                        <span className={`text-xl font-bold italic tracking-tighter text-white group-hover:text-${metric.color}-400 transition-colors`}>
                            {metric.value.toLocaleString(undefined, { style: 'currency', currency: 'AED', notation: 'compact' })}
                        </span>
                        {metric.subtext && (
                            <span className="text-[9px] font-bold italic text-zinc-600 mt-1">
                                {metric.subtext}
                            </span>
                        )}
                    </div>
