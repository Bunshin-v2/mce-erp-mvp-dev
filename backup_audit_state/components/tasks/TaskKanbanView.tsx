...
import { Plus, MoreHorizontal, Search, Filter, GripVertical, Clock, AlertCircle, CheckCircle2 } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-[10px] text-zinc-600 font-bold uppercase tracking-widest pl-8">Drag and drop tasks to update their status</p>
      </div>
      ...
      <GlassPanel className="p-4 flex flex-wrap items-center gap-6 border-white/5">
         <div className="flex items-center gap-4 flex-1">
            <div className="flex-1 max-w-xs">
               <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1.5 block">Priority</label>
               <select className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-[10px] text-zinc-300 outline-none focus:border-emerald-500/30 font-bold uppercase tracking-wider">
            ...
            <div className="flex-1 max-w-xs">
               <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1.5 block">Categories</label>
               <select className="w-full bg-black/40 border border-white/10 rounded-lg px-3 py-2 text-[10px] text-zinc-300 outline-none focus:border-emerald-500/30 font-bold uppercase tracking-wider">
                  <option>Select categories...</option>
               </select>
            </div>
            <div className="flex-1">
               <label className="text-[8px] font-black text-zinc-600 uppercase tracking-[0.2em] mb-1.5 block">Search</label>
               <div className="relative">
                  <Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600" />
                  <input placeholder="Search tasks..." className="w-full bg-black/40 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-[10px] text-white outline-none focus:border-emerald-500/30 font-bold uppercase tracking-wider" />
               </div>
            </div>
         </div>
         <button className="px-6 py-2 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black text-zinc-500 uppercase tracking-widest hover:text-white transition-all flex items-center gap-2">
            ...
            <div key={status} className="flex flex-col space-y-4">
              <div className={`flex items-center justify-between p-4 rounded-xl border border-white/5 bg-black/20 relative overflow-hidden group`}>
                ...
                <div className="flex items-center gap-3">
                   <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-white">{config.label}</h3>
              ...
              <div className="flex-1 space-y-4 min-h-[600px] bg-white/[0.01] rounded-3xl border border-dashed border-white/5 p-4">
                {columnTasks.map(task => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                       <h4 className="text-sm font-bold text-zinc-200 group-hover:text-white transition-colors uppercase tracking-tight leading-snug">{task.title}</h4>
                       <GripVertical size={14} className="text-zinc-800 group-hover:text-zinc-600" />
                    </div>
                    
                    <div className="flex flex-wrap gap-1.5 mb-4">
                       <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded border tracking-widest font-mono ${getPriorityColor(task.priority)}`}>
                          {task.priority}
                       </span>
                       {task.categories?.map(cat => (
                         <span key={cat.id} className="text-[8px] font-black uppercase px-2 py-0.5 rounded bg-white/5 border border-white/5 text-zinc-500 font-mono tracking-widest" style={{ color: cat.color }}>
                            {cat.name}
                         </span>
                       ))}
                    </div>

                    <div className="flex items-center justify-between pt-4 border-t border-white/5 text-[9px] font-mono font-bold text-zinc-600 uppercase">
                ...
                {columnTasks.length === 0 && (
                  <div className="h-full flex flex-col items-center justify-center py-20 opacity-20 grayscale scale-90">
                     ...
                     <p className="text-[9px] font-black uppercase tracking-[0.2em] text-zinc-500 mb-1">No {config.label} tasks</p>
                     <p className="text-[8px] font-bold uppercase text-zinc-700">Drop tasks here or</p>
                     <button onClick={() => onAddTask(status)} className="mt-4 text-[9px] font-black text-emerald-500 hover:underline uppercase tracking-widest">Add Task</button>
                  </div>
                )}
              </div>
            </div>
