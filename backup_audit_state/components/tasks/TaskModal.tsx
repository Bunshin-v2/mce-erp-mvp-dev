...
export const TaskModal: React.FC<TaskModalProps> = ({ onClose, onUpdate, categories, task }) => {
    ...
    const handleSave = async () => {
        ...
        let error;
        if (task) {
            const { error: err } = await supabase.from('tasks').update(taskData).eq('id', task.id);
            error = err;
        } else {
            const { data, error: err } = await supabase.from('tasks').insert([taskData]).select().single();
            error = err;
            ...
            <div className="bg-[#0f172a] border border-white/10 rounded-2xl w-full max-w-xl overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-300">
                <div className="p-6 border-b border-white/5 flex justify-between items-center bg-white/5">
                    <h2 className="text-xl font-black text-white tracking-tight flex items-center">
                        <CheckSquare className="mr-3 text-[#33CCCC]" size={20} />
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <input
                                ...
                                className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#33CCCC]/50 transition-all font-bold"
                            />
                        </div>
                        ...
                        <div>
                            ...
                            <textarea
                                ...
                                rows={3}
                                className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#33CCCC]/50 transition-all resize-none"
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <select
                                    ...
                                    className="w-full bg-slate-900 border border-white/10 rounded-xl px-4 py-2 text-sm text-white focus:outline-none focus:border-[#33CCCC]/50 transition-all"
                                >
                        ...
                        <div>
                            ...
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full bg-slate-900 border border-white/10 rounded-xl py-2 pl-10 pr-4 text-sm text-white focus:outline-none focus:border-[#33CCCC]/50 transition-all"
                                />
                            </div>
                        </div>

                        <div>
                            <label className="text-[10px] font-black uppercase tracking-widest text-slate-500 block mb-2">Categories</label>
                            <div className="flex flex-wrap gap-2">
                                {categories.map(cat => (
                                    <button
                                        ...
                                        className={`px-3 py-1.5 rounded-lg text-xs font-bold border transition-all flex items-center ${selectedCategories.includes(cat.id)
                                                ? 'border-[#33CCCC] text-[#33CCCC] bg-[#33CCCC]/10'
                ...
                <div className="p-6 bg-white/5 border-t border-white/10 flex justify-end space-x-3">
                    <button
                        onClick={onClose}
                        className="px-6 py-2 bg-white/5 text-slate-400 rounded-xl font-bold hover:bg-white/10 hover:text-white transition-all"
                    >
                        Cancel
                    </button>
                    <button
                        ...
                        className="px-8 py-2 bg-[#33CCCC] text-[#0f172a] rounded-xl font-bold hover:bg-[#20B2AA] transition-all disabled:opacity-50 shadow-[0_0_20px_rgba(51,204,204,0.2)]"
                    >
