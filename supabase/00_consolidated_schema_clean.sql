/**
 * MCE COMMAND CENTER - CONSOLIDATED DATABASE SCHEMA (CLEAN VERSION)
 *
 * Single-source-of-truth schema for the entire application.
 * This version is cleaner with proper DROP/CREATE logic.
 *
 * VERSION: 1.1
 * CREATED: 2026-01-28
...
-- Extract user role from JWT claims (Clerk integration)
...
-- Extract user ID from JWT claims (Clerk integration)
...
CREATE TABLE IF NOT EXISTS documents (
  ...
  version_number integer DEFAULT 1, -- Redundant but matching plan
  ...
  type text,
  sensitivity_level permission_tier DEFAULT 'L1', -- New field for RBAC
...
CREATE TABLE IF NOT EXISTS tenders (
  ...
  owner_user_id text,
  next_followup_at timestamptz, -- New field for workflow automation
  notes text,
  created_at timestamptz NOT NULL DEFAULT now(),
  updated_at timestamptz NOT NULL DEFAULT now(),
...
-- Tender Communication Events (Workflow tracking)
...
CREATE TABLE IF NOT EXISTS alerts (
  ...
  variable_id uuid,
  related_entity_type text, -- 'project', 'tender', 'document'
...
-- 8. Remaining Demo Policies (Permissive for now)
...
CREATE POLICY "Tenant isolation for embeddings" ON document_embeddings FOR ALL USING (
  tenant_id IN (SELECT tenant_id FROM profiles WHERE clerk_user_id = auth.uid()::text)
) WITH CHECK (
  tenant_id IN (SELECT tenant_id FROM profiles WHERE clerk_user_id = auth.uid()::text)
);
